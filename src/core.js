...
function calculatePreparationTime(number) {
  const time = 3
  const preparationTime = number * time
...
function totalTimeSpent(layersNumbers, minutes) {
  const preparationTime = calculatePreparationTime(layersNumbers)
  const totalTime = preparationTime + minutes
  return totalTime
}
