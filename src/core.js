...
function remainingBakeTime(timePassedInOven) {
  timePassedInOven = Math.max(timePassedInOven, 0)
  return bakingTime() - timePassedInOven
}
...
function totalTimeSpent(layers, timePassedInOven) {
  return  calculatePreparationTime(layers) + bakingTime() - remainingBakeTime(timePassedInOven)
}
