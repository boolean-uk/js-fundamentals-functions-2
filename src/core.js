...
function remainingBakeTime(numberOfMinutes) {
  const minutesLeft = bakingTime() - numberOfMinutes
  return minutesLeft
}
...
function calculatePreparationTime(numberOfLayers) {
  const timeToPrepareALayer = 3
  const minutesToPrepare = numberOfLayers * timeToPrepareALayer
...
function totalTimeSpent(numberOfLayers, remainingBakeTime) {
  const totalMinutesSpent = calculatePreparationTime(numberOfLayers) + remainingBakeTime
