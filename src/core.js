...
function remainingBakeTime(minsinoven) {
  const result = bakingTime() - minsinoven
  return result
}
console.log(remainingBakeTime(4))
...
function calculatePreparationTime(numoflayers) {
  const preparation = numoflayers * 3
  return preparation
}
...
function totalTimeSpent(numoflayers, minsinoven) {
  const total = calculatePreparationTime(numoflayers) + minsinoven
  return total
}
console.log(totalTimeSpent(5, 8))
