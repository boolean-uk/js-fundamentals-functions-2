...
function remainingBakeTime(ovenTime){
   const timeRemaining = bakingTime() - ovenTime
...
function calculatePreparationTime(numberOfLayers){
  ...
  const timeToPrep = numberOfLayers * 3
  return timeToPrep
}
...
function totalTimeSpent (howManyLayers, howLongBaking){
  const whenIsItCakeTime = calculatePreparationTime(howManyLayers) + howLongBaking
  return whenIsItCakeTime
}
