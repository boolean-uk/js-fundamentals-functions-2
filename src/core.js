...
function remainingBakeTime(timeInOven) {
  const timeLeft = bakingTime() - timeInOven
  return timeLeft
}
...
function calculatePreparationTime(numberOfLayers) {
  const oneLayer = 3
  const prepTime = numberOfLayers * oneLayer
  return prepTime
}
...
function totalTimeSpent(amountOfLayers, cookingTime) {
  return calculatePreparationTime(amountOfLayers) + cookingTime
}
