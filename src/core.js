...
function remainingBakeTime(minutes) {
  const totaltime = bakingTime() - minutes
  return totaltime
}
...
function calculatePreparationTime(numberOflayers) {
  const cakePrep = numberOflayers * 3
  return cakePrep
}
...
function totalTimeSpent(numberOflayers, minutes) {
  const totalTimeSpent = calculatePreparationTime(numberOflayers) + minutes
