...
function remainingBakeTime(minutesPassed) {
  const timeRemaining = bakingTimeV - minutesPassed
...
function totalTimeSpent(layers, timeLeft) {
  const timeSoFar = calculatePreparationTime(layers) + timeLeft
  return timeSoFar
}

totalTimeSpent(2, remainingBakeTime(10))
