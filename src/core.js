...
function remainingBakeTime(number) {
  const result = bakingTime() - number
  return result
}
console.log(remainingBakeTime(25))
...
function totalTimeSpent(layersNum, numOfMins) {
  const total = calculatePreparationTime(layersNum) + numOfMins
  return total
}
