...
function remainingBakeTime(minutesInOven) {
  const remainingMinutes = bakingTime() - minutesInOven
...
function calculatePreparationTime(NumberOfLayers) {
  const minutesToPrepareCake = 3 * NumberOfLayers
  return minutesToPrepareCake
}
...
function totalTimeSpent(numberOfLayers, numberOfMinutesBaking) {
  const totalTimeSpentBaking =
    calculatePreparationTime(numberOfLayers) + numberOfMinutesBaking
  return totalTimeSpentBaking
}
