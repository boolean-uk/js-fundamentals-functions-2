...
function remainingBakeTime(numberOfMinutes){
  let result = bakingTime() - numberOfMinutes
    return result
} 
...
function calculatePreparationTime(numberOfLayers){
  let layerTime = numberOfLayers * 3
  return layerTime
} 
...
function totalTimeSpent(numberOfLayers, result) {
  const preparationTime = calculatePreparationTime(numberOfLayers);
  const totalTime = preparationTime + result;
