...
function parseRequest(req) {
  ...
  if (!req.trim()) {
    return request; // Return empty request object if the input is empty
  }
...
function parseBody(body) {
  ...
  // Otherwise, parse the body string as JSON and return it
...
function extractQuery(path) {
  // Split the path string by '?' to separate path and query
  const parts = path.split('?');
  // If there's no query string, return null
  if (parts.length !== 2) {
    return null;
  }
  // Otherwise, parse the query string and return it as an object
...
// i had to use chatgpt for all of this
