...
const { remainingBakeTime } = require("../core")

function timerStatus (remaingTime) {
  if (remaingTime === 0) {
    return "Phil's cake is ready!" 
  } else if (remaingTime === undefined ) {
...
function estimatePrepTime(arr, prepAhead) {
  let estimatePrepTime = null
  for (let i = 0; i < arr.length; i++) {
    if (prepAhead === undefined) {
      estimatePrepTime += 2 
    } else {
      estimatePrepTime += prepAhead
    }
  } 
  return estimatePrepTime
}
...
function calculateQuantities(ingredientsList, cakeLayers) {
  ...
  if (ingredientsList.includes('sugar')) {
    list.sugar = 100 * cakeLayers
  } else {
    list.sugar = 0
  }
  if (ingredientsList.includes('eggs')) {
    list.eggs = 2 * cakeLayers
  } else {
    list.eggs = 0
  }
  return list
...
function improveRecipe(object, portions) {
  ...
  for (const ingredient in object) {
    newRecipe[ingredient] = object[ingredient] * portions
  }
  return newRecipe
}
