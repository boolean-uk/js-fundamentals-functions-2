...
function estimatePrepTime(ingredientsArray, prepTimePerIngredient) {
  if (prepTimePerIngredient === undefined) {
    prepTimePerIngredient = 2
  }

  return ingredientsArray.length * prepTimePerIngredient
}
...
function calculateQuantities(ingredientList, numberOfCakeLayers) {
  let sugar = 0
  let eggs = 0

  for (const element of ingredientList) {
    if (element === 'sugar') {
      sugar += numberOfCakeLayers * 100
      continue
    } else if (element === 'eggs') {
      eggs += numberOfCakeLayers * 2
    }
  }

  const sugarEggs = { sugar: sugar, eggs: eggs }
...
function improveRecipe(ingredientQuantityObject, numberOfPortions) {
  const newAndImproved = {}

  for (const [key, value] of Object.entries(ingredientQuantityObject)) {
    newAndImproved[key] = value * numberOfPortions
  }
