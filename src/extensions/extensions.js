...
function timerStatus(timerTime) {
  if (timerTime === 0) {
...
function calculateQuantities(ingredients, cakeLayers) {
  ...
  for (let i = 0; i < ingredients.length; i++) {
    if (ingredients[i] === 'sugar') {
      sugarQuantity = cakeLayers * 100
    }
    if (ingredients[i] === 'eggs') {
      eggsQuantity = cakeLayers * 2
    }
  }
  return { sugar: sugarQuantity, eggs: eggsQuantity }
}
...
function improveRecipe(ingredientList, portionNum) {
  const tmp = Object.keys(ingredientList)
  for (let i = 0; i < tmp.length; i++) {
    ingredientList[tmp[i]] *= portionNum
  }
