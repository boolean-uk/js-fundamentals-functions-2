...
function estimatePrepTime(ingredientsArray, ingredientPrepTime) {
  // console.log(`2, `, ingredientsArray.length)
  if (typeof ingredientPrepTime === `undefined`) {
    return ingredientsArray.length * 2
  } else {
    return ingredientsArray.length * ingredientPrepTime
  }
}

console.log(`2,`, estimatePrepTime(['sugar', 'milk', 'flour', 'eggs'], 10))

// 3. Phil needs to know the quantity of sugar and eggs to use! Create a function
...
function calculateQuantities(ingredientsList, cakeLayers) {
  ...
  if (ingredientsList.indexOf('sugar') !== -1) {
    quantity.sugar = cakeLayers * 100
  } else {
    quantity.sugar = 0
  }
  if (ingredientsList.indexOf('eggs') !== -1) {
    quantity.eggs = cakeLayers * 2
  } else {
    quantity.eggs = 0
  }
  return quantity
...
console.log(`3, `, calculateQuantities(['milk', 'sugar'], 2))
...
function improveRecipe(ingredientsList, portions) {
  const keys = Object.getOwnPropertyNames(ingredientsList)
  const values = Object.values(ingredientsList)
  const newIngredientsList = {}
  for (let i = 0; i < values.length; i++) {
    values[i] *= portions
    newIngredientsList[keys[i]] = values[i]
  }
  return newIngredientsList
}

console.log(`4, `, improveRecipe({ sugar: 100, eggs: 3, flour: 200 }, 1))
