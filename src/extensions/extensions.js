...
function timerStatus(timeLeftOnTimer) {
  if (timeLeftOnTimer === 0) {
    return "Phil's cake is ready!"
  } else if (timeLeftOnTimer > 0) {
...
function calculateQuantities(ingredients, numberOfLayers) {
  const result = { sugar: 0, eggs: 0 }

  for (let i = 0; i < ingredients.length; i++) {
    if (ingredients[i] === 'sugar') {
      result.sugar = 100 * numberOfLayers
    } else if (ingredients[i] === 'eggs') {
      result.eggs = 2 * numberOfLayers
    }
  }
  return result
}
...
function improveRecipe(ingredientsForOnePortion, numberOfportionsWanted) {
  const ingredientArray = Object.entries(ingredientsForOnePortion)
  for (let i = 0; i < ingredientArray.length; i++) {
    let ingredientAmount = ingredientArray[i][1]
    ingredientAmount = ingredientAmount * numberOfportionsWanted

    ingredientArray[i][1] = ingredientAmount
  }

  const newRecipe = Object.fromEntries(ingredientArray)
