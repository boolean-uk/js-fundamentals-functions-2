...
function timerStatus(mins) {
  ...
  } else if (mins !== 0 && typeof mins === 'number') {
...
function estimatePrepTime(array, time) {
  if (typeof time !== 'number') {
    time = 2
  }
  const totalPrepTime = array.length * time
  return totalPrepTime
}

console.log(estimatePrepTime(['sugar', 'eggs', 'milk'], ''))
...
function improveRecipe(ingredients, portion) {
  const eggsCount = portion * ingredients.eggs
  const sugarCount = portion * ingredients.sugar
  const milkCount = portion * ingredients.milk
  const flourCount = portion * ingredients.flour

  const IngredientsObj = {}

  if (ingredients.eggs) {
    IngredientsObj.eggs = eggsCount
  }
  if (ingredients.sugar) {
    IngredientsObj.sugar = sugarCount
  }
  if (ingredients.milk) {
    IngredientsObj.milk = milkCount
  }
  if (ingredients.flour) {
    IngredientsObj.flour = flourCount
  }
  return IngredientsObj
}
