...
function estimatePrepTime(ingredients, prepTimePerIngredient = 2) {
  ...
  const totalPrepTime = totalIngredients * prepTimePerIngredient
  console.log(`Estimated preparation time: ${totalPrepTime} minutes`)
...
function calculateQuantities(ingredients, numLayers) {
  const sugarAmount = ingredients.includes('sugar') ? numLayers * 100 : 0
  const eggsAmount = ingredients.includes('eggs') ? numLayers * 2 : 0
  console.log(`Sugar needed: ${sugarAmount}g, Eggs needed: ${eggsAmount}`)
  return { sugar: sugarAmount, eggs: eggsAmount }
}
...
function improveRecipe(recipe, portions) {
  ...
  console.log(`Improved recipe for ${portions} portions:`, improvedRecipe)
...
console.log(estimatePrepTime(['sugar', 'milk', 'eggs'], 2))
