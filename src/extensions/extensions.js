...
function timerStatus(remainingMins) {
  if (typeof(remainingMins) !== 'number')
    return 'You didn\'t set a timer!'
  return remainingMins <= 0 ? 'Phil\'s cake is ready!' : 'The cake is still baking!'
}
...
function estimatePrepTime(ingredients, prepTime) {
  prepTime = typeof(prepTime) !== 'number' ? 2 : prepTime
...
function calculateQuantities(ingredients, layers) {
  ...
  for (var elm of ingredients)
    if (_outObj[elm] !== undefined)
      _outObj[elm] = (elm === 'sugar' ? 100 : 2) * layers
...
function improveRecipe(keyIngredients, portions) {
  const _outObj = {...keyIngredients}

  for (const [key, elm] of Object.entries(_outObj)) {
    if (typeof(_outObj[key]) !== 'number') continue
    _outObj[key] = elm * portions
  }
