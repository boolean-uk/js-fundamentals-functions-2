...
function timerStatus(timer) {
  ...
  if (timer > 0) return 'The cake is still baking!'
...
function estimatePrepTime(ingredients, prep = 2) {
  return prep * ingredients.length
}
...
function calculateQuantities(ingredients, layers) {
  const sugarAmount = ingredients.filter((x) => x === 'sugar').length
  const eggsAmount = ingredients.filter((x) => x === 'eggs').length
  return {
    sugar: sugarAmount * 100 * layers,
    eggs: eggsAmount * 2 * layers
  }
}
...
function improveRecipe(ingredients, portions) {
  return Object.fromEntries(
    Object.entries(ingredients).map(([k, v]) => [k, v * portions])
  )
}
