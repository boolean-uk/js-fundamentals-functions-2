...
function calculateQuantities(ingredientsArray, numOfLayers) {
  const ingredientList = {}
  if (ingredientsArray.includes('sugar')) {
    ingredientList.sugar = 100 * numOfLayers
  }
  if (ingredientsArray.includes('sugar') === false) {
    ingredientList.sugar = 0
  }
  if (ingredientsArray.includes('eggs')) {
    ingredientList.eggs = 2 * numOfLayers
  }
  if (ingredientsArray.includes('eggs') === false) {
    ingredientList.eggs = 0
  }
  console.log(ingredientList)
  return ingredientList
}

calculateQuantities(['milk', 'sugar', 'eggs'], 3)
calculateQuantities(['milk', 'eggs'], 2)
calculateQuantities(['milk', 'sugar'], 2)
...
function improveRecipe(ingredientObject, numOfPortions) {
  const newQuantity = {}
  const eggsTotal = ingredientObject.eggs * numOfPortions
  const milkTotal = ingredientObject.milk * numOfPortions
  const sugarTotal = ingredientObject.sugar * numOfPortions
  const flourTotal = ingredientObject.flour * numOfPortions
  if (Object.prototype.hasOwnProperty.call(ingredientObject, 'eggs') === true) {
    newQuantity.eggs = eggsTotal
  }
  if (Object.prototype.hasOwnProperty.call(ingredientObject, 'milk') === true) {
    newQuantity.milk = milkTotal
  }
  if (
    Object.prototype.hasOwnProperty.call(ingredientObject, 'sugar') === true
  ) {
    newQuantity.sugar = sugarTotal
  }
  if (
    Object.prototype.hasOwnProperty.call(ingredientObject, 'flour') === true
  ) {
    newQuantity.flour = flourTotal
  }
  return newQuantity
}

console.log(improveRecipe({ eggs: 2, milk: 100, sugar: 250, flour: 160 }, 3))
