...
function estimatePrepTime(
  arrayOfIngredients,
  prepTimePerIngredientInMinutes = 2
) {
  let totalPrepTime = 0
  totalPrepTime = arrayOfIngredients.length * prepTimePerIngredientInMinutes
...
function calculateQuantities(listOfIngredients, numberOfCakeLayers) {
  const quantities = { sugar: 0, eggs: 0 }
  if (listOfIngredients.includes('sugar')) {
    quantities.sugar = 100 * numberOfCakeLayers
  }
  if (listOfIngredients.includes('eggs')) {
    quantities.eggs = 2 * numberOfCakeLayers
  }
  return quantities
}
...
function improveRecipe(ingredientsWithQuantity, numberOfPortions) {
  const keys = Object.keys(ingredientsWithQuantity)
  for (let i = 0; i < keys.length; i++) {
    ingredientsWithQuantity[keys[i]] =
      ingredientsWithQuantity[keys[i]] * numberOfPortions
  }
