...
function estimatePrepTime(ingredients, timeForEach) {
  if (timeForEach === undefined) {
    timeForEach = 2
  }
  const totalPrepTime = ingredients.length * timeForEach
...
function calculateQuantities(ingredients, layers) {
  const amountSugar = ingredients.includes('sugar') ? 100 * layers : 0
  const amountEggs = ingredients.includes('eggs') ? 2 * layers : 0
  return { sugar: amountSugar, eggs: amountEggs }
}
