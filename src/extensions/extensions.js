...
function estimatePrepTime(ingredient, time)
{
  if (!time)
  {
...
function calculateQuantities(ingredientList, layers)
{
  const returnValue = 
  {
    sugar: 0,
  ...
  for (let i = 0; i < ingredientList.length; i++)
  {
    ...
    {
      returnValue.sugar = 100 * layers;
    }
    if (ingredientList[i] === "eggs")
    {
      returnValue.eggs = 2 * layers;
    }
  }
