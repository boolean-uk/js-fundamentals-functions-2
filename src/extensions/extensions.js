...
function estimatePrepTime(arr, mins) {
  if (isNaN(mins)) {
    mins = 2
  }
  let output = 0
...
function calculateQuantities(ingredientArr, layers) {
  const output = {}
  if (ingredientArr.includes('sugar')) {
    output.sugar = 100 * layers
  } else {
    output.sugar = 0
  }
  if (ingredientArr.includes('eggs')) {
    output.eggs = 2 * layers
  } else {
    output.eggs = 0
  }
  return output
...
function improveRecipe(ingredientObj, servings) {
  const outputObj = Object.create(ingredientObj)
  for (const prop in ingredientObj) {
    outputObj[prop] = ingredientObj[prop] * servings
  }
  return outputObj
}
