...
function timerStatus(remainingMinsLeft) {
  if (remainingMinsLeft === undefined) {
...
function estimatePrepTime(ingredients, prepTimePerIngredigents = 2) {
  const arrayOfIngredients = ['sugar', 'milk', 'flour', 'eggs'];
  const numOfIngredients = ingredients.length;
const totalPrepTime = numOfIngredients * prepTimePerIngredigents
...
function calculateQuantities(listOfIngredigents, numsOfLayers) {
  const sugarPerLayer = 100;
  const eggsPerLayer = 2;
  let sugarNeeded = 0;
  let eggsNeeded = 0;

  for (let i = 0; i < listOfIngredigents.length; i++) {
    const ingredient = listOfIngredigents[i];
    if (ingredient === 'sugar') {
      sugarNeeded += sugarPerLayer * numsOfLayers;
    }
    if (ingredient === 'eggs') {
      eggsNeeded += eggsPerLayer * numsOfLayers;
  }
}
...
function improveRecipe(recipe, portions) {
  ...
  for (const [ingredient, quantity] of Object.entries(recipe)) {
    improvedRecipe[ingredient] = quantity * multiplier;
  }
