...
const estimatePrepTime = (arr, prepTime) => {
  const ingredients = []
  if (!prepTime) {
    prepTime = 2
  }
...
const calculateQuantities = (arr, layers) => {
  const list = { sugar: 0, eggs: 0 }
  const ingredients = arr.map((item) => item.toLowerCase())
  if (ingredients.indexOf('sugar') >= 0) {
    list.sugar = 100 * layers
  }
...
const improveRecipe = (obj, portions) => {
  Object.keys(obj).forEach((key) => (obj[key] *= portions))
  return obj
}
