...
function estimatePrepTime(ingredients, prepTime) {
  if (typeof prepTime === 'number') {
    let toPrep = ingredients.length * prepTime
    return toPrep
  } else if (typeof prepTime !== 'number') {
    return ingredients.length * 2
  }
}
const food = ['sugar', 'milk', 'flour', 'eggs']
console.log(food.includes('butter'))
// console.log(estimatePrepTime(food))
...
function calculateQuantities(ingredients, layers) {
  ...
  if (ingredients.includes('sugar') && ingredients.includes('eggs')) {
    ...
    ingredients.includes('eggs') === false
  ) {
    ingredientsNeeded.sugar = 100 * layers
  } else if (
    ingredients.includes('sugar') === false &&
    ingredients.includes('eggs') === true
  ) {
    ingredientsNeeded.eggs = 2 * layers
  }
...
// console.log(calculateQuantities('ingredients,food, 3))
...
const foods2 = { eggs: 2, milk: 100, sugar: 200 }
...
console.log(improveRecipe(foods2, 3))
