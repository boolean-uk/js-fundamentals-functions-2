...
function calculateQuantities(ingredients, layers) {
  let sugar = ingredients.includes('sugar') ? 100 * layers : 0
  let eggs = ingredients.includes('eggs') ? 2 * layers : 0
  return { sugar, eggs }
}
...
function improveRecipe(ingredients, portions) {
  ...
  //Object.keys(ingredients).forEach((ingredient) => ingredient *= portions)
  return ingredients
}
