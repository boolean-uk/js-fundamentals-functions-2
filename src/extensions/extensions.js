...
function estimatePrepTime(ingredients, minPer) {
  return minPer === undefined
    ? ingredients.length * 2
    : ingredients.length * minPer
}
...
function calculateQuantities(ingredients, layers) {
  ...
  for (const key in ingredients) {
    if (ingredients[key] === 'sugar') {
      sugar = true
    }
    if (ingredients[key] === 'eggs') {
      eggs = true
    }
  }

  return {
    sugar: sugar ? 100 * layers : 0,
    eggs: eggs ? 2 * layers : 0
  }
}
