...
function estimatePrepTime(arr, prepTime) {
...
function calculateQuantities(list, layers) {
  const sugarNeeded = list.includes('sugar') ? 100 * layers : 0
  const eggsNeeded = list.includes('eggs') ? 2 * layers : 0
  const quantities = { sugar: sugarNeeded, eggs: eggsNeeded }
  return quantities
}
...
function improveRecipe(list, portions) {
  const newList = {}
  for (const item in list) {
    newList[item] = list[item] * portions
  }
  return newList
}
