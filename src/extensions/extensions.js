...
function timerStatus(timerRemaningMinutes) {
  if (timerRemaningMinutes > 0) {
    return 'The cake is still baking!'
  } else if (timerRemaningMinutes === 0) {
...
function estimatePrepTime(listOfIngredients, prepTimePerIngredient = 2) {
  return listOfIngredients.length * prepTimePerIngredient
}
...
function calculateQuantities(listOfIngredients, nrOfCakeLayers) {
  const ingredients = { sugar: 0, eggs: 0 }

  if (listOfIngredients.includes('sugar')) {
    ingredients.sugar = 100 * nrOfCakeLayers
  }
  if (listOfIngredients.includes('eggs')) {
    ingredients.eggs = 2 * nrOfCakeLayers
  }
  return ingredients
}
...
function improveRecipe(ingredients, nrOfPortions) {
  Object.entries(ingredients).forEach(([key, value]) => {
    ingredients[key] = value * nrOfPortions
  })
  return ingredients
}
