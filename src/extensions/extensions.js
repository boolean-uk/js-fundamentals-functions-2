...
function timerStatus(remainingTime = undefined) {
  switch (true) {
    case remainingTime === undefined:
...
function estimatePrepTime(ingredientArray, prepTimePerIngredient = 2) {
  return ingredientArray.length * prepTimePerIngredient
}
...
function calculateQuantities(ingredientList, numLayers) {
  const result = {}
  result.sugar = ingredientList.includes('sugar') ? 100 * numLayers : 0
  result.eggs = ingredientList.includes('eggs') ? 2 * numLayers : 0
  return result
}
...
function improveRecipe(quantitiesDict, numPortions) {
  for (const key in quantitiesDict) {
    quantitiesDict[key] *= numPortions
  }
