...
function estimatePrepTime(ingredients, prepTimePerIngredient = 2) {
  ...
  return totalIngredients * prepTimePerIngredient
}
...
function calculateQuantities(ingredients, numLayers) {
  ...
  const sugarAmount = ingredients.includes('sugar') ? sugarPerLayer * numLayers : 0
  const eggsAmount = ingredients.includes('eggs') ? eggsPerLayer * numLayers : 0
