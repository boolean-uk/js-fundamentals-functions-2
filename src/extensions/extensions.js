...
function timerStatus(remainingMinute) {
...
function calculateQuantities(ingredients, layers) {
  ...
  for (const ingredient of ingredients) {
    // Check if the ingredient is 'sugar', then add 100g  per layer
    if (ingredient.toLowerCase() === 'sugar') {
      sugarNeed += 100 * layers
    }
    // Check if the ingredient is 'eggs', then add 2 eggs per layer
    if (ingredient.toLowerCase() === 'eggs') {
      eggsNeed += 2 * layers
    }
  }

  return { sugar: sugarNeed, eggs: eggsNeed }
}
...
function improveRecipe(recipe, portion) {
