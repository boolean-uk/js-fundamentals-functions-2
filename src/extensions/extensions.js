...
function estimatePrepTime(ingredients, prepTime) {
  ...
  for (let i = 0; i < ingredients.length; i++) {
    if (prepTime === undefined || prepTime <= 0) {
      totalTime += 2; // Assuming 2 minutes prep time per ingredient if not provided
    } else {
      totalTime += prepTime; // Adding provided prep time per ingredient
    }
  }
...
function improveRecipe(recipe, portions) {
  const originalPortions = 1;
  const updatedRecipe = {};

  for (const ingredient in recipe) {
    if (recipe.hasOwnProperty(ingredient)) {
      updatedRecipe[ingredient] = recipe[ingredient] * (portions / originalPortions);
    }
  }
