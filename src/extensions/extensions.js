...
function estimatePrepTime(ingArr, timePerItem = 2) {
  let prepTime = 0
  ingArr.forEach((e) => {
    prepTime += timePerItem
  })
  return prepTime
}
...
function calculateQuantities(ingArr, layers) {
  return {
    sugar: ingArr.includes('sugar') ? 100 * layers : 0,
    eggs: ingArr.includes('eggs') ? 2 * layers : 0
  }
}
...
function improveRecipe(recipe, n) {
  const improved = {}
  for (const [key, value] of Object.entries(recipe)) {
    improved[key] = value * n
  }
  return improved
}
