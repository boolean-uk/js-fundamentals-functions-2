...
function estimatePrepTime(array, timePer = 2) {
  return array.length * timePer
}
...
function calculateQuantities(array, layers) {
  const sugar = array.includes('sugar') ? 100 * layers : 0
  const eggs = array.includes('eggs') ? 2 * layers : 0
  return { sugar, eggs }
}
