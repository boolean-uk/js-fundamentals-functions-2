...
const timerStatus = (minutes) => {
  if (minutes === 0) {
    ...
    return "Negative minutes are not possible!"
  }else {
...
const estimatePrepTime = (ingredients, prepTime = 2) => {
...
const calculateQuantities = (ingredients, layers) => {
  let ret = { sugar: 0, eggs: 0 }
  if (ingredients.indexOf('sugar') !== -1) {
      ret.sugar = 100 * layers
  }
  if (ingredients.indexOf('eggs') !== -1) {
      ret.eggs = 2 * layers
  }
