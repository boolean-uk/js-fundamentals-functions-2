...
function estimatePrepTime(arrIng, prepTime) {
  if (typeof prepTime === 'number') {
    return arrIng.length * prepTime
  } else {
    return arrIng.length * 2
  }
}
...
function calculateQuantities(listIng, layers) {
  const Ing = {
    sugar: 0,
    eggs: 0
  }
  for (let i = 0; i < listIng.length; i++) {
    if (listIng[i] === 'sugar') {
      Ing.sugar += layers * 100
    } else if (listIng[i] === 'eggs') {
      Ing.eggs += layers * 2
    } else {
      continue
    }
...
function improveRecipe(object, portions) {
  for (let i = 0; i < Object.keys(object).length; i++) {
    object[Object.keys(object)[i]] *= portions
  }
  return object
}
