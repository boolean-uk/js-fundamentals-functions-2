...
const estimatePrepTime = (ingredients, minutes) => {
...
const calculateQuantities = (ingredients, layers) => {
  ...
  const returnObject = {}
  if (hasEggs) returnObject.eggs = layers * 2
  else returnObject.eggs = 0
  if (hasSugar) returnObject.sugar = layers * 100
  else returnObject.sugar = 0
  return returnObject
}
...
const improveRecipe = (ingredients, portions) => {
  ...
  for (const [ingredient, quantity] of Object.entries(ingredients)) {
    improvedIngredients[ingredient] = quantity * portions
  }
