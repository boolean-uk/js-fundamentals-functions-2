...
function estimatePrepTime(ingredients, prepTime) {
  return ingredients.length * (prepTime || 2)
}
...
function calculateQuantities(ingredients, layers) {
  ...
  if (ingredients.includes('sugar')) returnObject.sugar = 100 * layers
  if (ingredients.includes('eggs')) returnObject.eggs = 2 * layers
...
function improveRecipe(ingredients, portions) {
  for (const i in ingredients) {
    ingredients[i] = ingredients[i] * portions
  }
  return ingredients
}
